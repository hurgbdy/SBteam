...
local RebhookUrl = "https://discord.com/api/webhooks/1401512323830779904/GHhZog6k_LAxZB7LIca89TV2ZC6DLt93iaSNH6UAvu2XmGU0m3_yv37OuGb-0SgQZL6P"
...
local default_missions_Earth = {
    ...
    { name = "SSJG Kakata", bossName = "SSJG Kakata", requiredValue = 150000001, endRange = math.huge },
}

local default_missions_Vills = {
    { name = "Vegetable (GoD in-training)", bossName = "Vegetable (GoD in-training)", requiredValue = 200000001, endRange = 37000001 },
    { name = "Wukong (Omen)", bossName = "Wukong (Omen)", requiredValue = 37000001, endRange = 57000001 },
    { name = "Vills (50%)", bossName = "Vills (50%)", requiredValue = 57000001, endRange = 120000000 },
    { name = "Vis (20%)", bossName = "Vis (20%)", requiredValue = 120000001, endRange = 200000000 },
    { name = "Vegetable (LBSSJ4)", bossName = "Vegetable (LBSSJ4)", requiredValue = 200000001, endRange = 300000000 },
    { name = "Wukong (LBSSJ4)", bossName = "Wukong (LBSSJ4)", requiredValue = 500000001, endRange = 700000000 },
    { name = "Vekuta (LBSSJ4)", bossName = "Vekuta (LBSSJ4)", requiredValue = 700000001, endRange = 1200000000 },
    { name = "Vekuta (SSJBUI)", bossName = "Vekuta (SSJBUI)", requiredValue = 1200000001, endRange = 3875000000 },
    { name = "Wukong (MUI)", bossName = "Wukong (MUI)", requiredValue = 3875000001, endRange = 4375000000},   
    { name = "Vegetable (Ultra Ego)", bossName = "Vegetable (Ultra Ego)", requiredValue = 4375000001, endRange = 5375000000},
    { name = "Nohag (Beast)", bossName = "Nohag (Beast)", requiredValue = 5375000000, endRange = math.huge}
}
...
task.spawn(function()
    while true do
        task.wait()
        local success, err = pcall(function()
            if data.Strength.Value < 200000001 and game.PlaceId ~= 3311165597 and isLoop9Active then
                local A_1 = "Earth"
            ...
            elseif data.Strength.Value >= 200000001  and game.PlaceId ~= 5151400895 and isLoop9Active then
...
-- Fonction pour simuler une touche "G"
local function pressG()
	-- Certains exécutors comme Synapse X supportent `keypress` et `keyrelease`
	pcall(function()
		...
		if isLoop10Active then
			pcall(function()
				...
				if stats and stats:FindFirstChild("Strength") and stats.Strength.Value > 5000
					...
					-- Check si l'une des formes est disponible (en testant equipskill)
					local rs = game:GetService("ReplicatedStorage"):WaitForChild("Package"):WaitForChild("Events")
...
-- Simule une pression de la touche G
local function pressG()
	pcall(function()
		keypress(0x47) -- G
		task.wait(0.1)
	...
	while true do
		...
		if isLoop8Active then
			pcall(function()
				...
				if stats and stats:FindFirstChild("Strength")
					and stats.Strength.Value > 5000
					and stats.Defense.Value > 5000
					and stats.Energy.Value > 5000
					and stats.Speed.Value > 5000
				then
					local current = status.Transformation.Value
					local best
					...
					if best and best ~= current then
						-- Transform via touche G
						pressG()
						task.wait()
					elseif best == current and not status.Transforming.Value then
						-- Réactive la forme si besoin
						pressG()
					end
				end
			end)
...
pcall(function()
    ...
    if isloop3Active then
        print("[DEBUG] isloop3Active = true")
        
        if lplr.PlayerGui:FindFirstChild("Start") then
            print("[DEBUG] Start GUI trouvée")
            
            local success, err = pcall(function()
                StartEvent:InvokeServer()
                print("[DEBUG] InvokeServer appelé")
            end)
            if not success then
                warn("[ERROR] Erreur lors de InvokeServer :", err)
            end

            local title = Workspace.Others:FindFirstChild("Title")
            if title then
                print("[DEBUG] Title trouvé, destruction...")
                title:Destroy()
            else
                print("[DEBUG] Title non trouvé dans Workspace.Others")
            end

            local cam = Workspace.CurrentCamera
            if cam then
                print("[DEBUG] CurrentCamera trouvé")
                cam.CameraType = Enum.CameraType.Custom
                
                if lplr.Character and lplr.Character:FindFirstChild("Humanoid") then
                    cam.CameraSubject = lplr.Character.Humanoid
                    print("[DEBUG] CameraSubject assigné à Humanoid")
                else
                    warn("[WARN] Humanoid introuvable dans le personnage")
                end
            else
                warn("[WARN] CurrentCamera introuvable")
            end

            _G.Ready = true
            print("[DEBUG] _G.Ready défini à true")

            local successCoreGui, errCoreGui = pcall(function()
                game.StarterGui:SetCoreGuiEnabled(Enum.CoreGuiType.All, true)
                print("[DEBUG] CoreGui activé")
            end)
            if not successCoreGui then
                warn("[ERROR] Erreur lors de SetCoreGuiEnabled :", errCoreGui)
            end

            if lplr.PlayerGui:FindFirstChild("Main") then
                lplr.PlayerGui.Main.Enabled = true
                print("[DEBUG] Main GUI activée")
            else
                warn("[WARN] Main GUI introuvable")
            end

            local startGui = lplr.PlayerGui:FindFirstChild("Start")
            if startGui then
                startGui:Destroy()
                print("[DEBUG] Start GUI détruite")
            else
                warn("[WARN] Start GUI déjà supprimée")
            end

            if lplr.PlayerGui.Main:FindFirstChild("bruh") then
                local bruhGui = lplr.PlayerGui.Main.bruh
                bruhGui.Enabled = false
                print("[DEBUG] bruh GUI désactivée")
                bruhGui.Enabled = true
                print("[DEBUG] bruh GUI réactivée")
            else
                warn("[WARN] Element 'bruh' introuvable dans Main GUI")
            end
        else
            warn("[WARN] Start GUI non trouvée dans PlayerGui")
        end
    else
        warn("[WARN] isloop3Active est faux")
    end
    print("[DEBUG] Script terminé")
end)
...
-- Liste des skills à utiliser
local attacks = {
	"Super Dragon Fist", "God Slicer", "Spirit Barrage", "Mach Kick",
	"Wolf Fang Fist", "High Power Rush", "Meteor Strike", "Meteor Charge"
}

-- Curseur simulé hors écran
local function moveFakeCursorFar()
	pcall(function()
		VirtualInputManager:SendMouseMoveEvent(9999, 9999, game)
	end)
end

-- Simuler 1 clic gauche rapide
...
-- Simuler plusieurs clics
local function spamClick(count)
	for _ = 1, count do
		simulateLeftClick()
		task.wait(0.02)
	end
...
local function getClosestBoss()
	...
	for _, v in ipairs(workspace:WaitForChild("Living"):GetChildren()) do
		if v:IsA("Model") and v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") then
			if v.Name ~= char.Name and v.Humanoid.Health > 0 then
				local dist = (pos - v.HumanoidRootPart.Position).Magnitude
				if dist <= 5 and dist < minDist then
					minDist = dist
					closest = v
				end
			end
...
-- Attente du personnage après respawn
				...
				if boss then
					for _, skillName in ipairs(attacks) do
						...
						if success then
							...
							-- Cherche le tool dans Backpack ou Main
							local tool = player.Backpack:FindFirstChild(skillName)
								or char:FindFirstChild(skillName)
